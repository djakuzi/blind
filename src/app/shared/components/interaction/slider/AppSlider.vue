...
const maxIndex = computed(() =>
  Math.max(0, props.count - 1),
);

const activeIndex = computed(() =>
  Math.min(
    Math.max(props.modelValue, 0),
    maxIndex.value,
  ),
);
...
const sliderMaxWidth = computed(() =>
  LibStyle.toSizeValue(props.maxWidth),
);

const sliderItemWidth = computed(() =>
  LibStyle.toSizeValue(props.itemWidth),
);

const sliderItemMaxWidth = computed(() =>
  LibStyle.toSizeValue(props.itemMaxWidth),
);

const sliderItemGap = computed(() =>
  resolveSpaceValue(props.itemGap),
);

const sliderContentDotsGap = computed(() =>
  resolveSpaceValue(
    props.contentDotsGap
      ?? sizeConfig.value.contentDotsGap,
  ),
);

const sliderDotsGap = computed(() =>
  resolveSpaceValue(
    props.dotsGap
      ?? sizeConfig.value.dotsGap,
  ),
);

const sliderDotSize = computed(() =>
  LibStyle.toSizeValue(props.dotSize),
);

const sliderDotColor = computed(() =>
  resolveColorValue(props.dotColor),
);

const sliderActiveDotColor = computed(() =>
  resolveColorValue(props.activeDotColor),
);

const sliderInactiveScale = computed(() =>
  Math.min(
    1,
    Math.max(
      0,
...
const sliderInactiveOpacity = computed(() =>
  Math.min(
    1,
    Math.max(
      0,
...
function resolveVisualDragOffset(
  offset: number,
) {
  const isFirst =
    activeIndex.value === 0;
...
function updateDragOffset(
  ...
  dragFrameId = requestAnimationFrame(() => {
    ...
    dragOffset.value =
      resolveVisualDragOffset(offset);
  });
}
...
.app-slider__item {
  ...
  transform:
    scale(v-bind(sliderInactiveScale));
